 ...
 * 异步请求时的回调处理
 */
public abstract class Okhttp3Callback<T> {
    ...
    public static void onFailureDefault(Call call, Exception e) {
        Okhttp3Utils.logger.error("[Okhttp3请求错误] url:{}", call.request().url(), e);
    }

    public static void onErrorDefault(Response response) {
        Okhttp3Utils.logger.error("[Okhttp3服务器错误] url:{}, code:{}, message:{}", new Object[]{response.request().url(), response.code(), response.message()});
    }
}
