...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            ...
            <!-- /.series-header -->
            <!-- form start -->
            <form id="form1" action="${springMacroRequestContext.contextPath}/jewelryType/${jewelryType.id}" role="form">
                <div class="box-body">
                    <div class="row">
                        ...
                        <div class="col-xs-offset-1 col-xs-5 col-md-4">
                            <div class="form-group">
                                ...
                                <input class="form-control" name="name" id="name" placeholder="请输入名称"
                                       value="${jewelryType.name}"
                                       data-rule="required"
                                       data-msg-required="名称不能为空" disabled
                                >
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-offset-1 col-xs-5 col-md-4">
                            <div class="form-group">
                                ...
                                <textarea class="form-control remark_style" id="remark" name="remark" placeholder="类别描述">${jewelryType.remark}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(function () {
        ...
        $.submitForm("#form1", ".btn-submit", "PUT", ["${_csrf.parameterName}", "${_csrf.token}"],
                function (data) {
                    $.redirect("${springMacroRequestContext.contextPath}/jewelryType/list", function () {
                    });
                }, function (err) {
