    ...
    <div class="col-xs-12">
        <div class="box">
            ...
            <div class="box-body">
                ...
                [/@searchable]
                <div>
                [#--[@authorize ifAnyGranted="diamond:add"]--]
                [#--<a class="btn btn-flat bg-olive" href="${springMacroRequestContext.contextPath}/diamond/add"--]
                [#--redirect>新增</a>--]
                [#--[/@authorize]--]
                </div>
    ...
    $(function () {
        ...
        var dataTable = $form.DataTable({
            ...
            'stateSaveParams': function (settings, data) {
                ...
                $('.form-control.status')[0].value = data.columns[6].search.search;
            },
            'ordering': true,
            'info': true,
            'autoWidth': false,
            ...
            'columns': [
                ...
                },
                {
                    data: 'id', orderable: false, render: function (data, type, full, meta) {
                        var operations = [];
                    [@authorize ifAnyGranted="diamond:view"]
                        operations.push('<a redirect href="${springMacroRequestContext.contextPath}/diamond/view/' + data + '">详情</a>');
                    [/@authorize]
                    [@authorize ifAnyGranted="diamond:edit"]
                        operations.push('<a redirect href="${springMacroRequestContext.contextPath}/diamond/edit/' + data + '">编辑</a>');
                    [/@authorize]
                    [#--[@authorize ifAnyGranted="diamond:delete"]--]
            ...
            'ajax': {
                ...
                'url': '${springMacroRequestContext.contextPath}/diamond/pagination',
                'type': 'POST',
        ...
        $form.on('click', '.btn-delete', function () {
            var $this = $(this);
            var id = $this.data('id');
            var url = "${springMacroRequestContext.contextPath}/diamond/" + id;
