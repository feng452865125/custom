            ...
            <div class="box-body">
                [@searchable id="searchable" targetId="example"]
                    <div class="input-group">
                        <span class="input-group-addon">花头编号</span>
                ...
                [/@searchable]
                <div>
                [#--[@authorize ifAnyGranted="flowerHead:add"]--]
                [#--<a class="btn btn-flat bg-olive" href="${springMacroRequestContext.contextPath}/flowerHead/add"--]
                [#--redirect>新增</a>--]
                [#--[/@authorize]--]
                </div>
    ...
    $(function () {
        ...
        var dataTable = $form.DataTable({
            ...
            'columns': [
                ...
                {data: 'level', name: 'level', orderable: false},
                {
                    data: 'id', orderable: false, render: function (data, type, full, meta) {
                        var operations = [];
                    [@authorize ifAnyGranted="flowerHead:view"]
                        operations.push('<a redirect href="${springMacroRequestContext.contextPath}/flowerHead/view/' + data + '">详情</a>');
                    [/@authorize]
                    [@authorize ifAnyGranted="flowerHead:edit"]
                        operations.push('<a redirect href="${springMacroRequestContext.contextPath}/flowerHead/edit/' + data + '">编辑</a>');
                    [/@authorize]
                    [#--[@authorize ifAnyGranted="flowerHead:delete"]--]
            ...
            'ajax': {
                ...
                'url': '${springMacroRequestContext.contextPath}/flowerHead/pagination',
                'type': 'POST',
        ...
        $form.on('click', '.btn-delete', function () {
            var $this = $(this);
            var id = $this.data('id');
            var url = "${springMacroRequestContext.contextPath}/flowerHead/" + id;
