...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            ...
            <!-- /.series-header -->
            <!-- form start -->
            <form id="form1" action="${springMacroRequestContext.contextPath}/orders/${orders.id}" role="form">
                <div class="box-body">
                    ...
                    <div class="row">
                        <div class="col-xs-offset-1 col-xs-5 col-md-4">
                            <div class="form-group">
                                ...
                                [#else]
                                    <input class="form-control" name="ktCode" value="" >
                                [/#if]
                            </div>
                        </div>
                        <div class="col-xs-5 col-md-4">
                            <div class="form-group">
                                <label>钻石GIA证书号</label>
                                <input class="form-control" value="${(orders.stoneZsbh)!"无"}" disabled>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-xs-offset-1 col-xs-5 col-md-4">
                            <div class="form-group">
                                <label>证书编号</label>
                                <input class="form-control" name="zsBh"value="${(orders.parts.exZsBh)!"无"}">
                            </div>
                        </div>
                    </div>
                    [/#if]
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-xs-offset-1 col-md-1">
                            [#if !orders.product.ktCode??]
                                <button type="submit" class="btn btn-primary btn-submit">修改客定码</button>
                            [#else]
                                <button type="submit" class="btn btn-primary btn-submit2">修改证书号</button>
                            [/#if]
                        </div>
...
<script>
    $(function () {
        $.submitForm("#form1", ".btn-submit", "PUT", ["${_csrf.parameterName}", "${_csrf.token}"],
                function (data) {
                    $.redirect("${springMacroRequestContext.contextPath}/orders/list", function () {
                    });
                }, function (err) {
        ...
        [#--$.submitForm("#form1", ".btn-submit2", "POST", ["${_csrf.parameterName}", "${_csrf.token}"],--]
                [#--function (data) {--]
                    [#--$.redirect("${springMacroRequestContext.contextPath}/orders/list", function () {--]
                    [#--});--]
                [#--}, function (err) {--]
