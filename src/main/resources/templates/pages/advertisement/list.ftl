            ...
            <div class="box-body">
                [@searchable id="searchable" targetId="example"]
                    <div class="input-group">
                        ...
                        <input name="name" class="form-control" placeholder="请输入图片名称查询">
                    </div>
                [/@searchable]
                <div>
                    [@authorize ifAnyGranted="advertisement:add"]
                        <a class="btn btn-flat bg-olive"
                           href="${springMacroRequestContext.contextPath}/advertisement/add"
                           redirect>新增</a>
                    [/@authorize]
                </div>
    ...
    $(function () {
        ...
        var dataTable = $form.DataTable({
            ...
            'columns': [
                ...
                {data: 'createDate', name: 'createDate'},
                {
                    data: 'id', orderable: false, render: function (data, type, full, meta) {
                        var operations = [];
                        [@authorize ifAnyGranted="advertisement:edit"]
                            operations.push('<a redirect href="${springMacroRequestContext.contextPath}/advertisement/edit/' + data + '">编辑</a>');
                        [/@authorize]
                        [@authorize ifAnyGranted="advertisement:view"]
                            operations.push('<a redirect href="${springMacroRequestContext.contextPath}/advertisement/view/' + data + '">详情维护</a>');
                        [/@authorize]
                        [@authorize ifAnyGranted="advertisement:delete"]
            ...
            'ajax': {
                ...
                'url': '${springMacroRequestContext.contextPath}/advertisement/pagination',
                'type': 'POST',
        ...
        $form.on('click', '.btn-delete', function () {
            var $this = $(this);
            var id = $this.data('id');
            var url = "${springMacroRequestContext.contextPath}/advertisement/" + id;
        ...
        dataTable.on('draw.dt', function () {
            ...
            var confirmOperation = function () {
                ...
                $.ajax({
                    url: "${springMacroRequestContext.contextPath}/advertisement/" + id + "/" + extUrl,
                    type: 'PATCH',
